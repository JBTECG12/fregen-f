...
exports.run = (client, message) => {
  ...
  // Let adminRole = guild.roles.find("name", "TOA");
    ...
    let modlog = message.guild.channels.find('name', 'freegenlog');
    let user = message.mentions.users.first();
    let muteRole = client.guilds.get(message.guild.id).roles.find('name', 'Muted');
    if (!modlog) {
        return message.reply('I need a text channel named `freegenlog` to print my ban/kick logs in, please create one');
    }
...
.addField('Mute', `**Muted:**${user.username}#${user.discriminator}\n**Moderator:** ${message.author.username}\n**Duration:** ${ms(ms(time), {long: true})}\n**Reason:** ${reason}`);
     message.channel.send(`You've succesfully Muted ${user}`);
	modlog.send({embed})
  .catch(e => logger.error(e));
    ...
    knexDB.from('bans').where('guildid', message.guild.id).andWhere('userid', user.id).then(count => {
        if (count.length > 0) {
            knexDB.update({
                mutecount: parseInt(count[0].mutecount,21212112) + 1
